...
******************************************************************/
#include "whi_nav2_bt_plugins/condition/is_charging_enough_condition.hpp"

namespace whi_nav2_bt_plugins
{
	IsChargingEnoughCondition::IsChargingEnoughCondition(const std::string &XmlTagName, const BT::NodeConfiguration &Conf)
		: BT::ConditionNode(XmlTagName, Conf),
		  state_service_("battery_state")
	{
		/// node version and copyright announcement
		std::cout << "\nWHI is battery charge enough bt node VERSION 00.01.1" << std::endl;
	...
	BT::NodeStatus IsChargingEnoughCondition::tick()
	{
		if (battery_state_client_)
		{
				...
				rclcpp::FutureReturnCode::SUCCESS)
			{
				...
				if (result->result)
				{
					if (result->state.state == whi_interfaces::msg::WhiBattery::STA_GOOD_FOR_TASK)
					{
...
{
	factory.registerNodeType<whi_nav2_bt_plugins::IsChargingEnoughCondition>("IsChargingEnough");
}
